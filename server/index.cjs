...
if (!ADMIN_TOKEN) {
  console.warn('Warning: ADMIN_TOKEN not set. Admin endpoints will require X-Admin-Token header to match this value.');
}
...
app.post('/api/admin/client_images/reorder', requireAdmin, async (req, res) => {
  try {
    const { order } = req.body; // expecting [{id, order_index},...]
...
// - These allow browser clients to perform limited actions (favorites, downloads,
//   analytics, view increments) without exposing the service-role key to the
//   browser. The server performs light validation (ensures gallery exists/is active)
// -----------------------------------------------------------------------------
...
app.post('/api/client/favorites', async (req, res) => {
  try {
    ...
    // ensure gallery exists and is active
